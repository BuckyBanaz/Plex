...
class DriverOrderTrackingController extends GetxController {
  ...
  void onInit() async {
    ...
    // Start periodic location sync every 60 seconds
  ...
  void _checkArrival() {
    ...
    debugPrint(
      'Distance: ${distance.toStringAsFixed(1)}m → Enabled: ${isActionButtonEnabled.value}',
    );
  }
  ...
  Future<void> _syncLocationToServer() async {
    ...
    try {
      ...
      if (orderId != null) {
        ...
        debugPrint(
          "Location synced to server: $orderId -> ${currentLatLng!.latitude}, ${currentLatLng!.longitude}",
        );
      } else {
